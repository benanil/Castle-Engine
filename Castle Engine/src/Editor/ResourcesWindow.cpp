...
#include "../Rendering/AssetManager.hpp"
#include "../Helper.hpp"
...
{
	using namespace CSCore; // string to hash
	...
	constexpr uint cmeshHash = StringToHash(".cmesh");
	...
	FolderTree* CreateTreeRec(FolderTree* parent, const std::filesystem::path& path)
	{
		...
		for (auto& directory : std::filesystem::directory_iterator(path))
		{
			...
			if (extension == ".png" or extension == ".jpg") {
				const uint hash = StringToHash(filePath.c_str());
